    ...
    thread_ws * ws)
{
  ...
  if(dest_msgs->num_epair_msgs == dest_msgs->max_epair_msgs) {
    ...
    gk_epair_t * big = gk_malloc(dest_msgs->max_epair_msgs * sizeof(*big), "big");
...
gk_epair_t * get_incoming_epair_bucket(
    thread_ws * * ws,
    int const bucket_id,
    int64_t * num_pairs)
{
  ...
  gk_epair_t * const restrict buffer = my_ws->epair_buffer;
  ...
  for(int t=0; t < my_ws->num_threads; ++t) {
    ...
    if(num_msgs > 0) {
      gk_epair_t const * const msgs = ws[t]->buckets[bucket_id].epair_msgs;
      for(int32_t m=0; m < num_msgs; ++m) {
        buffer[buffer_ptr++] = msgs[m];
      }
